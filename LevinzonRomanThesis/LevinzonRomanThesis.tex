...
\chapter{Research}
This chapters serves as an introduction to the Android development and gives the overview to the core technologies that will be used during the development.
...
The View and Presenter both are abstracted to an Interface, it makes all the layers testable and easily testable.\cite{arch-mvp}
...
All requirements specified in the Analysis were implemented along with the  couple new functionalities that were not a part  of the requirements specifications:
	1) A screen to view latest notifications received
	2)  Introduction of real-time features when receiving push-notifications when the application is in the foreground.
3) The functionality to send feedback from the users
4) Crash-Reporting and Analytics Services to support application future development.
...
Future development
StudyPad development is far from over.  The current iteration of the application had an epmhasis on content creation, sharing, and collaboration, with just a few tests available for users to study and learn. The real process of studying has yet to be researched and executed.
Future  StudyPad development will concentrate on how to improve this aspect of the application.
There also some small changes that could be  addressed such as:
1) Powerful Notes editor that would make notes creation easier.
